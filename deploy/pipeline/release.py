...
def validate(release):
    ...
    if runtime != json.loads(Path(__file__).with_name("runtime.json").read_text()):
        raise ValueError("Runtime changed: manually review/install the matching controller before deployment")
