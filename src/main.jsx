...
function SetExercise({session,exercise,save,onResolve}){const next=exercise.sets.find(s=>s.status==='pending');const [weight,setWeight]=useState(exercise.chosen_target_load??'');const [actualWeight,setActualWeight]=useState(next?.target_load??'');const [actualReps,setActualReps]=useState(next?.target_rep_min??'');const [rir,setRir]=useState('');useEffect(()=>{setActualWeight(next?.target_load??'');setActualReps(next?.target_rep_min??'');setRir('')},[next?.id]);const updateTarget=()=>save(`/api/sessions/${session.id}/exercises/${exercise.id}/target`,'PATCH',{chosenTargetLoad:Number(weight)});const log=()=>save(`/api/sessions/${session.id}/sets/${next.id}/log`,'POST',{actualLoad:Number(actualWeight),actualReps:Number(actualReps),rir:rir===''?null:Number(rir)});return <><p>Target: {exercise.prescription.workSetCount} work sets × {exercise.prescription.repMin}–{exercise.prescription.repMax} reps</p><label>Weight for remaining sets · {exercise.unit_snapshot}<span className="inline"><input type="number" value={weight} min="0" step={exercise.prescription.loadStep||1} onChange={e=>setWeight(e.target.value)}/><button type="button" onClick={updateTarget}>Apply</button></span></label><p className="muted">Suggested {exercise.suggestion_load??'not set'} {exercise.unit_snapshot||''} · logged targets and actuals stay unchanged</p><details><summary>Warm-ups &amp; work sets · {exercise.sets.filter(x=>x.status!=='pending').length} resolved</summary>{exercise.sets.map(s=><div className="setrow" key={s.id}><span>{s.kind==='warmup'?'Warm-up':`Work set ${s.prescribed_set_ordinal}`}<small>{s.target_load??'—'} {exercise.unit_snapshot} × {s.target_rep_min}{s.target_rep_max!==s.target_rep_min?`–${s.target_rep_max}`:''}</small></span><span>{s.status==='completed'?`✓ ${s.actual_load} × ${s.actual_reps}`:s.status==='skipped'?'Skipped':s.id===next?.id?'Next':'—'}</span></div>)}</details>{next?<section className="current"><h2>{next.kind==='warmup'?'Warm-up':`Work set ${next.prescribed_set_ordinal}`}</h2><p>Target: {next.target_load??'Choose load'} {exercise.unit_snapshot} × {next.target_rep_min}{next.target_rep_max!==next.target_rep_min?`–${next.target_rep_max}`:''}</p><div className="fields"><label>Actual {exercise.unit_snapshot}<input type="number" min="0" value={actualWeight} onChange={e=>setActualWeight(e.target.value)}/></label><label>Actual reps<input type="number" min="0" step="1" value={actualReps} onChange={e=>setActualReps(e.target.value)}/></label></div><label>Reps in reserve · optional<select value={rir} onChange={e=>setRir(e.target.value)}><option value="">Not recorded</option>{[0,1,2,3].map(x=><option key={x}>{x}</option>)}</select></label><button className="primary full" onClick={log}>Log set</button><button className="plain full" onClick={()=>save(`/api/sessions/${session.id}/sets/${next.id}/skip`,'POST',{})}>Skip set</button></section>:<button className="primary full" onClick={()=>onResolve('completed')}>Complete exercise</button>}</>}
